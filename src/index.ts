...
import { buildForkSessionSnapshotJsonl, sanitizeForkSnapshot, compressToolResults, stripBatchReadToolCalls } from "./snapshot/snapshot.js";
...
export { compressToolResults, stripBatchReadToolCalls };
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerFlag("flow-model-config", {
    description: "Select the flow model strategy configuration for this session.",
    type: "string",
  });
  pi.registerFlag("flow-mode", {
    description: "Temporarily switch the global flow model strategy.",
    type: "string",
  });
  ...
  pi.on("context", async (event) => {
    ...
    const { messages } = stripSteeringHintsFromMessages(inputMessages);
  ...
  pi.on("before_agent_start", async (event) => {
    ...
    return systemPrompt === undefined ? event : { ...event, systemPrompt };
  });
  ...
  pi.on("session_start", async (_event, ctx) => {
    ...
    const sessionTools = currentDepth === 0
      ? [createBatchReadTool()]
      : [
          createBatchTool(bashTracker, resolved.toolOptimize),
          createTimedBashToolDefinition(ctx.cwd),
          createBatchBashPollTool(bashTracker),
        ];

    for (const tool of sessionTools.filter((tool): tool is NonNullable<typeof tool> => tool != null)) {
  ...
  pi.registerTool({
    ...
    async execute(toolCallId: string, params: any, signal: AbortSignal, onUpdate: any, ctx: ExtensionContext) {
      if (!resolved) {
        return {
    ...
    renderCall: (args: any, theme: any) => renderFlowCall(args, theme),
    renderResult: (result: any, { expanded }: { expanded: boolean }, theme: any, args: any) => renderFlowResult(result, expanded, theme, args),
  });

  const pluginApi: PiAgentFlowAPI & Record<string, unknown> = {
