...
Use the helper scripts instead of raw `npm link` / global reinstall commands.
...
This toggles the global install to your local checkout. Verify the output shows a symlink (`-> /path/to/repo`).

### Daily dev loop
Once linked locally, you do not need to switch on every edit:
```shell
npm run build
# Restart pi after rebuilding so it picks up dist/ through the symlink
```
For prompt-dump debugging, you can start Pi with the helper script:
```shell
...
npm ls -g pi-agent-flow
```
Verify `npm ls -g pi-agent-flow` shows a version number rather than a symlink.

> 🚫 **Do not run `pi update` while linked locally.** It reinstalls the published package globally and overwrites the local symlink. Switch back to the published version first, then run `pi update`.
