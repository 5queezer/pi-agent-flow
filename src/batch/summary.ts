/**
 * Pure batch operation summary helpers.
 *
 * Shared by runtime event rendering and TUI rendering.
 */
...
function formatOpSummary(op: { o: string; p: string; e?: unknown[]; c?: string; s?: number; l?: number }): string {
	...
	switch (op.o) {
		...
		case "ls": return `ls ${shortPath}`;
		case "find": return `find ${shortPath}`;
		case "grep": return `grep ${shortPath}`;
		case "delete": return `delete ${shortPath}`;
		default: return `${op.o} ${shortPath}`;
	}
}
