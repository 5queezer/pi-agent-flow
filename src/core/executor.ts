...
async function confirmProjectFlowsIfNeeded(
  ...
  const lines = projectFlows.map((f) => `• ${f.name} — ${f.description}`);
  const dirLine = projectFlowsDir ? `\n\nDirectory: ${projectFlowsDir}` : "";
  const body =
    `The following project-local flow(s) are about to run:\n\n${lines.join("\n")}${dirLine}`;

  if (hasUI) {
    const approved = await uiConfirm("Run project-local flows?", body);
    return approved ? { ok: true } : { ok: false };
  }

  logWarn(
    `[pi-agent-flow] Project-local flows requested without UI confirmation available: ${projectFlows
      .map((f) => f.name)
      .join(", ")}`,
  );
  return {
    ok: false,
    blocked:
      "Blocked: project-local flows requested, but no interactive UI confirmation is available in this session.",
  };
}

function shouldFailover(result: SingleResult): boolean {
  if (isFlowComplete(result)) return false;
  if (result.exitCode === 124 || result.exitCode === 143) return false;
  if (result.exitCode === 0 && !isFlowError(result)) return false;
  return true;
}
...
export async function executeFlows(
  ...
  if (
    cliFlowMode !== undefined &&
    cliFlowModelConfig !== undefined &&
    cliFlowMode !== cliFlowModelConfig
  ) {
    console.warn(
      ...
      allResults
        .map((r) => {
          const remainingSeconds =
            r.exitCode === -1 && typeof r.deadlineAtMs === "number"
        ...
        onUpdate: (partial) => {
          if (partial.details?.results[0]) {
            ...
            if (flowText !== undefined) {
              ...
              setLiveText(`collapsed#${index}`, flowText);
            }
  ...
  for (const result of results) {
    ...
    if (!so) {
      logWarn(
        `[pi-agent-flow] Flow result for toolCallId=${toolCallId} type=${result.type} has no structuredOutput — cache entry skipped. This means child flows will see placeholder text instead of compressed results.`,
      );
      continue;
    }
