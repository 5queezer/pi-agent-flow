...
import type { HatchetFlowPayload } from "./hatchet-runner.js";
import {
...
export { HATCHET_FLOW_TASK_NAME };
export type { HatchetFlowPayload };
...
export function validateSingleResult(value: unknown, source = "Hatchet result"): SingleResult {
	...
	for (const field of ["type", "agentSource", "intent", "aim", "stderr"] as const) {
		if (typeof value[field] !== "string") {
			throw new Error(`${source} returned an invalid SingleResult: "${field}" must be a string.`);
		}
	}
	if (!["user", "project", "bundled", "unknown"].includes(value.agentSource as string)) {
		throw new Error(`${source} returned an invalid SingleResult: "agentSource" must be one of user, project, bundled, or unknown.`);
	}
	...
	for (const field of ["input", "output", "cacheRead", "cacheWrite", "cost", "contextTokens", "turns", "toolCalls"] as const) {
		if (typeof value.usage[field] !== "number" || !Number.isFinite(value.usage[field])) {
			throw new Error(`${source} returned an invalid SingleResult: "usage.${field}" must be a finite number.`);
		}
	}
	if (value.usage.smoothedTps !== undefined && (typeof value.usage.smoothedTps !== "number" || !Number.isFinite(value.usage.smoothedTps))) {
		throw new Error(`${source} returned an invalid SingleResult: "usage.smoothedTps" must be a finite number when present.`);
	}
	for (const field of ["acceptance", "model", "stopReason", "errorMessage", "streamingText"] as const) {
		if (value[field] !== undefined && typeof value[field] !== "string") {
			throw new Error(`${source} returned an invalid SingleResult: "${field}" must be a string when present.`);
		}
	}
	...
	for (const field of ["startedAtMs", "deadlineAtMs"] as const) {
		if (value[field] !== undefined && (typeof value[field] !== "number" || !Number.isFinite(value[field]))) {
			throw new Error(`${source} returned an invalid SingleResult: "${field}" must be a finite number when present.`);
		}
	}
...
function awaitHatchetResult<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
	return new Promise<T>((resolve, reject) => {
		const timer = setTimeout(() => reject(new Error(`Hatchet did not return a result for ${HATCHET_FLOW_TASK_NAME} within ${timeoutMs}ms. Check Hatchet connectivity and ensure a worker is registered for ${HATCHET_FLOW_TASK_NAME}. Adjust ${PI_FLOW_HATCHET_RESULT_TIMEOUT_MS_ENV} if longer waits are expected.`)), timeoutMs);
		timer.unref?.();
		promise.then((value) => {
			clearTimeout(timer);
			resolve(value);
...
export async function submitHatchetTaskWithClient(client: HatchetTaskClient, payload: HatchetFlowPayload): Promise<SingleResult> {
...
export async function submitHatchetTaskWithSdk(sdk: HatchetSdkModule, taskName: string, payload: HatchetFlowPayload): Promise<SingleResult> {
	const clientFactory = asFunction(getProperty(sdk, "HatchetClient")) ?? asFunction(getProperty(sdk, "Hatchet")) ?? asFunction(getProperty(sdk, "default"));
	...
	const directRun = asFunction(getProperty(client, "run"));
	...
	const workflowRun = asFunction(getProperty(workflows, "run"));
	...
	const taskRun = asFunction(getProperty(tasks, "run")) ?? asFunction(getProperty(tasks, "execute"));
...
export class HatchetFlowRunner {
	constructor(private readonly submitTask: HatchetSubmitter = defaultSubmitHatchetTask) {}

	async run(options: RunFlowOptions, context?: { projectFlowsDir: string | null }): Promise<SingleResult> {
		...
		try {
			const result = validateSingleResult(await awaitHatchetResult(this.submitTask(HATCHET_FLOW_TASK_NAME, payload), timeoutMs), "Hatchet runner result");
