...
async function askViaDialogs(
   ...
   const dialogOpts: { timeout?: number; allowCancel?: boolean } = {};
   if (timeout) dialogOpts.timeout = timeout;
   if (!allowCancel) dialogOpts.allowCancel = false;
   ...
   return {
      ...
      parameters: Type.Object({
         ...
         allowCancel: Type.Optional(
            Type.Boolean({ description: "Allow the user to cancel without answering. Default: true" }),
         ),
      ...
      async execute(_toolCallId: string, params: AskParams, signal: AbortSignal | undefined, onUpdate: ((result: any) => void) | undefined, ctx: ExtensionContext) {
         ...
         if (options.length === 0) {
            ...
            const inputOpts: { timeout?: number; allowCancel?: boolean } = {};
            if (timeout) inputOpts.timeout = timeout;
            if (!allowCancel) inputOpts.allowCancel = false;
