...
describe("main agent tool restriction", () => {
	...
	it("restricts main agent to batch_read+flow+web+ask_user+explore when toolOptimize is true", async () => {
		...
		expect(calledWith).toEqual(["batch_read", "flow", "web", "ask_user", "explore"]);
	});
	...
	it("re-applies batch_read+flow+web on turn_start when optimized", async () => {
		...
		expect(lastCall).toEqual(["batch_read", "flow", "web", "ask_user", "explore"]);
	});
	...
	it("registers batch_read for main agent; batch/batch_bash_poll reserved for children", async () => {
		...
		// Main agent active tools: batch_read + flow + web + ask_user + explore (batch and batch_bash_poll registered but not active)
